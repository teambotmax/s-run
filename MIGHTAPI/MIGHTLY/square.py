...
class Square(object):
    ...
    """Object"""
    @loggedIn
    def sendVideoFooter(self, squareChatMid, path, link, icon, footer):
        objectId = self.sendSquareMessage(squareChatMid=squareChatMid, text=None, contentMetadata = {'VIDLEN': '60000','DURATION': '60000', 'AGENT_LINK': link, 'AGENT_ICON': icon, 'AGENT_NAME': footer}, contentType = 2).createdSquareMessage.message.id
        return self.uploadObjSquare(path=path, type='video', returnAs='bool', objId=objectId)

    @loggedIn
    def sendVideoWithFooter(self, squareChatMid, url, link, icon, footer):
        path = self.downloadFileURL(url, 'path')
        return self.sendVideoFooter(squareChatMid, link, icon, footer)
    @loggedIn
    ...
    @loggedIn
    def sendSquareImageFooter(self, squareChatMid, path, link, icon, footer):
        objectId = self.sendSquareMessage(squareChatMid=squareChatMid, text=None, contentMetadata = {'AGENT_LINK': link, 'AGENT_ICON': icon, 'AGENT_NAME': footer}, contentType = 1).createdSquareMessage.message.id
        return self.uploadObjSquare(path=path, type='image', returnAs='bool', objId=objectId)

    @loggedIn
    def sendSquareImageWithFooter(self, squareChatMid, url, link, icon, footer):
        path = self.downloadFileURL(url, 'path')
        return self.sendSquareImageFooter(squareChatMid, path, link, icon, footer)
    @loggedIn
    ...
    def sendSquareReplyMessage(self, relatedMessageId, squareChatMid, text, contentMetadata={}, contentType=0):
        ...
        return self.square.sendMessage(rq)
    @loggedIn
    def sendSquareReplyFooterMessage(self, relatedMessageId, squareChatMid, text,title=None, link=None, iconlink=None, contentMetadata={}, contentType=0):
        rq = SendMessageRequest()
        rq.squareChatMid = squareChatMid
        rq.squareMessage = SquareMessage()
        msg = self.generateReplyMessage(relatedMessageId)
    ...
    def sendSquareMention(self, to, mid, firstmessage='', lastmessage=''):
        ...
        self.sendSquareMessage(to, text, {'MENTION': str('{"MENTIONEES":' + str(arr) + '}')}, 0)
    ...
    def sendSquareMentionV2(self, to, text="", mids=[], isUnicode=False):
        ...
        self.sendSquareMessage(to, textx, {'MENTION': str('{"MENTIONEES":' + str(arr) + '}')}, 0)
