...
class Talk(object):
    ...
    """Extra"""
    @loggedIn
    def getAllOpenTicket(self):
        jarray = {}
        s = self.getGroupsV2(self.getGroupIdsJoined()[0:100])
        for g in s:
            if g.preventedJoinByTicket == False:
                jarray[g.id] = self.reissueGroupTicket(g.id)
        return jarray
    @loggedIn
    def boardcastMessage(self,message):
        for i in self.getGroupIdsJoined():
            try:
    ...
    def sendReplyMessage(self, relatedMessageId, to, text, contentMetadata={}, contentType=0):
        ...
        return self.talk.sendMessage(self._messageReq[to], msg)
    @loggedIn
    def sendReplyMention(self, id, to, text="", mids=[], isUnicode=False):
        arrData = ""
        arr = []
        mention = "@zeroxyuuki "
        if mids == []:
        ...
        self.sendReplyMessage(id,to, textx, {'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, 0)
    @loggedIn
    ...
    @loggedIn
    def sendMentionV3(self, id, to, text="", mids=[], isUnicode=False):
        arrData = ""
        arr = []
        mention = "@zeroxyuuki "
        if mids == []:
        ...
        if "@!" in text:
            ...
            raise Exception("Invalid mention position")
        self.sendReplyMessage(id, to, textx, contentMetadata={'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, contentType=0)

    @loggedIn
    def sendMentionV2(self, to, text="", mids=[], isUnicode=False, update=None):
        arrData = ""
        arr = []
        mention = "@zeroxyuuki "
        if mids == []:
        ...
        if "@!" in text:
            ...
            raise Exception("Invalid mention position")
        ctm = {'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}
        if update != None:
            ctm.update(update)
        self.sendMessage(to, textx, ctm, 0)
    ...
    @loggedIn
    def listAllFriend(self,type=0):
        list = self.talk.getAllContactIds()
        stringx = None
        if type == 0:
            stringx = []
            for friend in list:
                stringx.append({friend:str(self.getContact(friend).displayName)})
        elif type == 1:
            stringx = "[ LIST ALL FRIEND ]\n"
            for friend in list:
                stringx+= self.getContact(friend).displayName + "\n"
            stringx+= ""
        return stringx
    @loggedIn
