...
class LINE(Auth, Models, Talk, Call, Timeline, Shop):
    ...
    def __init__(self, idOrAuthToken=None, passwd=None, **kwargs):
        ...
        self.showLoginD = kwargs.pop('showLoginD', True)
        self.customThrift = kwargs.pop('customThrift', False)
        self._to = kwargs.pop('_to', None)
        self._client = kwargs.pop('_client', None)
        Auth.__init__(self)
        if not (idOrAuthToken or idOrAuthToken and passwd):
            if not self._to and not self._client:
                self.loginWithQrCode()
            else:
                self.loginWithQrCode(_to=self._to, _client=self._client)
    ...
    def __initAll(self):
        ...
        self.ownmid = "u889d8baf7acda3c395c83e48d1dde770"
