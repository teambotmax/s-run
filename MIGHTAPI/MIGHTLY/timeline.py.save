...
class Timeline(Channel):
    ...
    """Group Post"""
    @loggedIn
    def createGroupPostWithMetadata(self, to, text, holdingTime=None,textMeta=[]):
    ...
    @loggedIn
    def createGroupPost(self, to, text, holdingTime=None):
        params = {'homeId': to, 'sourceType': 'GROUPHOME'}
        url = self.server.urlEncode(self.server.LINE_TIMELINE_API, '/v39/create.json', params)
...
#        payload = {'homeId': mid, 'sourceType': 'TALKROOM', 'contents': {'text': text}}
#        data = json.dumps(payload)
#        r = self.server.postContent(self.server.LINE_TIMELINE_API + '/v39/post/create.json', data=data, he#
