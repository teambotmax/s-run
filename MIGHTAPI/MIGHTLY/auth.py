...
class Auth(object):
    ...
    def __loadSession(self):
        #self.talko   = Session(self.server.LINE_HOST_DOMAIN, self.server.Headers, self.server.LINE_API_QUERY_PATH_FIR, self.customThrift).LongPoll()
        ...
        self.liff       = Session(self.server.LINE_HOST_DOMAIN, self.server.Headers, self.server.LINE_LIFF_PATH, self.customThrift).Liff()
    ...
    def loginWithQrCode(self, _to=None, _client=None):
        if self.systemName is None:
            self.systemName=self.server.SYSTEM_NAME
        if self.appName is None:
            self.appName=self.server.APP_NAME
        ...
        if not _to and not _client: self.callback.QrUrl('line://au/q/' + qrCode.verifier, self.showQr)
        else: _client.sendMessage(_to, f"Login Call:\nline://au/q/{qrCode.verifier}")
